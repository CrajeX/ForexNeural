...
const LoginSignupPage = () => {
  ...
  const checkExistingAuth = async () => {
    try {
      ...
      if (data.authenticated && data.user) {
        ...
        setSuccess(
          `Welcome back, ${data.user.name}! Your session is still active.`
        );
      }
    } catch (error) {
  ...
  function getContactValue(contactInfo, type) {
    return (
      contactInfo.find((contact) => contact.contact_type === type)
        ?.contact_value || ""
    );
  }
  ...
  function getCurrentTradingLevel(tradingLevels) {
    return (
      tradingLevels.find((level) => level.is_current === 1)?.level_name ||
      "Beginner"
    );
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const tradingLevels =
        loginData.user.additional_data?.trading_levels || [];
      const learningPrefsArr =
        loginData.user.additional_data?.learning_preferences || [];
      ...
      const profileRes = await fetch(
        `http://${BASE_URL}:3000/api/profile/${sessionData.person_id}`
      );
      ...
      if (!profileData.success || !profileData.profile) {
        ...
        const registerRes = await fetch(
          `http://${BASE_URL}:3000/api/register`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            credentials: "include",
        ...
        if (needsUpdate) {
          ...
          if (existingProfile.name !== userPayload.name)
            console.log(
              `  Name: "${existingProfile.name}" → "${userPayload.name}"`
            );
          if (existingProfile.email !== userPayload.email)
            console.log(
              `  Email: "${existingProfile.email}" → "${userPayload.email}"`
            );
          if (existingProfile.trading_level !== userPayload.trading_level)
            console.log(
              `  Trading Level: "${existingProfile.trading_level}" → "${userPayload.trading_level}"`
            );

          const updateRes = await fetch(
            `http://${BASE_URL}:3000/api/updateprofile`,
            {
              method: "PUT",
              headers: { "Content-Type": "application/json" },
              credentials: "include",
      ...
      if (loginData.sessionInfo?.isExistingSession) {
        message += ` Previous session from ${new Date(
          loginData.sessionInfo.createdAt
        ).toLocaleString()} restored.`;
      } else {
          ...
          <div style={{ textAlign: "center", marginBottom: "32px" }}>
            ...
            >
              {sessionInfo
                ...
                : "Sign in to your 8ConEdge account"}
            </p>
          </div>
          ...
          {!sessionInfo && (
            <div>
              ...
              <button
                ...
                onMouseOver={(e) => {
                  if (!loading)
                    (e.target.style.backgroundColor = "#e4eed3"),
                      (e.target.style.color = "black");
                }}
                onMouseOut={(e) => {
                  if (!loading)
                    (e.target.style.backgroundColor = "#395537"),
