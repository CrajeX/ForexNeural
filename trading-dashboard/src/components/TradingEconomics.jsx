...
const TradingEconomicsHistory = () => {
  ...
  const [assetsError, setAssetsError] = useState(null);
  const [lastUpdated, setLastUpdated] = useState(null);
  const [rawDataSample, setRawDataSample] = useState(null);
  const [allFields, setAllFields] = useState([]);
  const BASE_URL = import.meta.env.VITE_API_BASE_URL || 'localhost';

  // Available data types with dark/light color pairs
  const dataTypes = [
    { value: 'cot', label: 'COT Data', darkColor: '#1f2937', lightColor: '#9ca3af' },
    { value: 'employment', label: 'Employment Change', darkColor: '#065f46', lightColor: '#6ee7b7' },
    { value: 'unemployment', label: 'Unemployment Rate', darkColor: '#92400e', lightColor: '#fbbf24' },
    { value: 'gdp', label: 'GDP Growth', darkColor: '#c2410c', lightColor: '#fb923c' },
    { value: 'mpmi', label: 'Manufacturing PMI', darkColor: '#166534', lightColor: '#4ade80' },
    { value: 'spmi', label: 'Services PMI', darkColor: '#1e40af', lightColor: '#60a5fa' },
    { value: 'retail', label: 'Retail Sales', darkColor: '#0f766e', lightColor: '#5eead4' },
    { value: 'inflation', label: 'Core Inflation', darkColor: '#a16207', lightColor: '#facc15' },
    { value: 'interest', label: 'Interest Rates', darkColor: '#b91c1c', lightColor: '#f87171' },
    { value: 'nfp', label: 'NFP (USD Only)', darkColor: '#1e3a8a', lightColor: '#93c5fd' }
  ];
  ...
  }, [selectedAsset, BASE_URL]);

  // Comprehensive field value extractor
  const extractFieldValue = (item, possibleFields, debug = false) => {
    if (debug) {
      console.log('🔍 Extracting from item:', item);
      console.log('🔍 Looking for fields:', possibleFields);
    }

    for (const field of possibleFields) {
      if (item.hasOwnProperty(field)) {
        const value = item[field];
        if (debug) console.log(`🔍 Found field '${field}' with value:`, value, typeof value);
        
        if (value !== null && value !== undefined && value !== '') {
          const numValue = parseFloat(value);
          if (!isNaN(numValue)) {
            if (debug) console.log(`✅ Successfully parsed '${field}':`, numValue);
            return numValue;
          }
        }
      }
    ...
    if (debug) console.log('❌ No valid value found in any field');
    return 0;
  };
  ...
  const processHistoricalData = (rawData, dataType, timeframe) => {
    ...
    console.log('🔧 Processing', rawData.length, 'items for type:', dataType);
    
    // Extract all unique field names for debugging
    const fieldNames = new Set();
    rawData.forEach(item => {
      Object.keys(item).forEach(key => fieldNames.add(key));
    });
    setAllFields(Array.from(fieldNames).sort());
    
    // Store sample for debugging
    setRawDataSample(rawData[0]);
    console.log('📊 Sample raw item:', rawData[0]);
    console.log('📊 All available fields:', Array.from(fieldNames));

    const processedData = rawData.map((item, index) => {
      const date = new Date(item.created_at);
      
      const formatDate = (date) => {
        if (['weekly'].includes(timeframe)) {
          return date.toLocaleDateString('en-US', { 
            month: 'short', 
            day: 'numeric',
            hour: '2-digit',
        ...
        return date.toLocaleDateString('en-US', { 
          year: 'numeric', 
          month: 'short', 
          ...(timeframe === 'monthly' || timeframe === 'weekly' ? { day: 'numeric' } : {})
        });
      };
      ...
      // Enhanced field mapping with more comprehensive field names
      switch (dataType) {
        case 'cot':
          dataPoint.actual = extractFieldValue(item, [
            'long_percent', 'longPercent', 'long_percentage', 'long_position', 'long_positions',
            'long', 'longs', 'long_pos', 'cot_long', 'commitment_long'
          ]);
          dataPoint.forecast = extractFieldValue(item, [
            'short_percent', 'shortPercent', 'short_percentage', 'short_position', 'short_positions',
            'short', 'shorts', 'short_pos', 'cot_short', 'commitment_short'
          ]);
          dataPoint.actualLabel = 'Long Positions %';
          dataPoint.forecastLabel = 'Short Positions %';
          break;

        case 'employment':
          dataPoint.actual = extractFieldValue(item, [
            'employment_change', 'actual_employment', 'employment', 'actual', 'change',
            'emp_change', 'employment_data', 'value', 'employment_value', 'jobs_change'
          ]);
          dataPoint.forecast = extractFieldValue(item, [
            'forecast', 'expected', 'prediction', 'forecast_employment', 'employment_forecast',
            'expected_employment', 'emp_forecast', 'predicted_value'
          ]);
          dataPoint.actualLabel = 'Actual Employment';
          dataPoint.forecastLabel = 'Forecast';
          break;

        case 'unemployment':
          dataPoint.actual = extractFieldValue(item, [
            'unemployment_rate', 'rate', 'actual', 'unemployment', 'jobless_rate',
            'actual_rate', 'unemployment_value', 'value', 'current_rate'
          ]);
          dataPoint.forecast = extractFieldValue(item, [
            'forecast', 'expected', 'prediction', 'forecast_rate', 'unemployment_forecast',
            'expected_rate', 'predicted_rate', 'forecast_unemployment'
          ]);
          dataPoint.actualLabel = 'Actual Rate %';
          dataPoint.forecastLabel = 'Forecast %';
          break;

        case 'gdp':
          dataPoint.actual = extractFieldValue(item, [
            'gdp_growth', 'growth', 'actual', 'gdp', 'economic_growth',
            'actual_gdp', 'gdp_value', 'value', 'growth_rate', 'gdp_rate'
          ]);
          dataPoint.forecast = extractFieldValue(item, [
            'forecast', 'expected', 'prediction', 'forecast_gdp', 'gdp_forecast',
            'expected_growth', 'predicted_gdp', 'forecast_growth'
          ]);
          dataPoint.actualLabel = 'Actual GDP %';
          dataPoint.forecastLabel = 'Forecast %';
          break;

        case 'mpmi':
          dataPoint.actual = extractFieldValue(item, [
            'manufacturing_pmi', 'mpmi', 'pmi', 'actual', 'manufacturing',
            'man_pmi', 'actual_pmi', 'value', 'manufacturing_value', 'service_pmi'
          ]);
          dataPoint.forecast = extractFieldValue(item, [
            'forecast', 'expected', 'prediction', 'forecast_pmi', 'pmi_forecast',
            'expected_pmi', 'predicted_pmi', 'manufacturing_forecast'
          ]);
          dataPoint.actualLabel = 'Actual MPMI';
          dataPoint.forecastLabel = 'Forecast';
          break;

        case 'spmi':
          dataPoint.actual = extractFieldValue(item, [
            'service_pmi', 'services_pmi', 'spmi', 'pmi', 'actual', 'services',
            'serv_pmi', 'actual_pmi', 'value', 'services_value'
          ]);
          dataPoint.forecast = extractFieldValue(item, [
            'forecast', 'expected', 'prediction', 'forecast_pmi', 'pmi_forecast',
            'expected_pmi', 'predicted_pmi', 'services_forecast'
          ]);
          dataPoint.actualLabel = 'Actual SPMI';
          dataPoint.forecastLabel = 'Forecast';
          break;

        case 'retail':
          dataPoint.actual = extractFieldValue(item, [
            'retail_sales', 'sales', 'actual', 'retail', 'retail_value',
            'actual_sales', 'value', 'sales_data', 'retail_data'
          ]);
          dataPoint.forecast = extractFieldValue(item, [
            'forecast', 'expected', 'prediction', 'forecast_sales', 'retail_forecast',
            'expected_sales', 'predicted_sales', 'sales_forecast'
          ]);
          dataPoint.actualLabel = 'Actual Sales %';
          dataPoint.forecastLabel = 'Forecast %';
          break;

        case 'inflation':
          // Enhanced field mapping for core inflation with debug
          const inflationDebug = dataType === 'inflation' && index === 0;
          dataPoint.actual = extractFieldValue(item, [
            'core_inflation', 'inflation', 'cpi', 'actual', 'inflation_rate',
            'actual_inflation', 'value', 'inflation_value', 'core_cpi',
            'cpi_rate', 'consumer_price_index', 'price_index', 'core_rate',
            'inflation_data', 'cpi_data', 'core_inflation_rate', 'actual_cpi',
            'current_inflation', 'monthly_inflation', 'annual_inflation'
          ], inflationDebug);
          dataPoint.forecast = extractFieldValue(item, [
            'forecast', 'expected', 'prediction', 'forecast_inflation', 'inflation_forecast',
            'expected_inflation', 'predicted_inflation', 'cpi_forecast',
            'forecast_cpi', 'expected_cpi', 'predicted_cpi', 'inflation_expected',
            'core_forecast', 'forecast_rate', 'expected_rate'
          ], inflationDebug);
          dataPoint.actualLabel = 'Actual Inflation %';
          dataPoint.forecastLabel = 'Forecast %';
          break;

        case 'interest':
          // Enhanced field mapping for interest rates with debug
          const interestDebug = dataType === 'interest' && index === 0;
          dataPoint.actual = extractFieldValue(item, [
            'interest_rate', 'rate', 'actual', 'interest', 'policy_rate',
            'actual_rate', 'value', 'interest_value', 'base_rate', 'fed_rate',
            'central_bank_rate', 'official_rate', 'benchmark_rate', 'key_rate',
            'current_rate', 'repo_rate', 'discount_rate', 'prime_rate',
            'overnight_rate', 'target_rate', 'cash_rate', 'bank_rate'
          ], interestDebug);
          dataPoint.forecast = extractFieldValue(item, [
            'change_in_interest', 'change', 'rate_change', 'interest_change',
            'change_value', 'rate_diff', 'interest_diff', 'rate_adjustment',
            'interest_adjustment', 'policy_change', 'rate_movement',
            'change_in_rate', 'interest_move', 'rate_shift', 'basis_points',
            'bps_change', 'forecast', 'expected', 'prediction'
          ], interestDebug);
          dataPoint.actualLabel = 'Interest Rate %';
          dataPoint.forecastLabel = 'Change %';
          break;

        case 'nfp':
          dataPoint.actual = extractFieldValue(item, [
            'actual_nfp', 'nfp', 'actual', 'payrolls', 'employment',
            'non_farm', 'jobs', 'value', 'nfp_value', 'payroll_data'
          ]);
          dataPoint.forecast = extractFieldValue(item, [
            'forecast', 'expected', 'prediction', 'forecast_nfp', 'nfp_forecast',
            'expected_nfp', 'predicted_nfp', 'payroll_forecast'
          ]);
          dataPoint.actualLabel = 'Actual NFP';
          dataPoint.forecastLabel = 'Forecast';
          break;
        ...
        default:
          dataPoint.actual = extractFieldValue(item, [
            'value', 'actual', 'data', 'amount', 'level', 'rate', 'index'
          ]);
          dataPoint.forecast = extractFieldValue(item, [
            'forecast', 'expected', 'prediction', 'target', 'estimate'
          ]);
          dataPoint.actualLabel = 'Actual Value';
          dataPoint.forecastLabel = 'Forecast';
      }

      // Calculate variance and result
      if (dataPoint.forecast !== undefined && dataPoint.actual !== undefined && dataPoint.forecast !== 0) {
        dataPoint.variance = dataPoint.actual - dataPoint.forecast;
        const percentVariance = Math.abs(dataPoint.variance / dataPoint.forecast) * 100;
        
        if (percentVariance < 1) {
          dataPoint.result = 'Met';
        } else if (dataPoint.variance > 0) {
          dataPoint.result = 'Beat';
        } else {
          dataPoint.result = 'Miss';
        }
      }
      ...
      if (index === 0) {
        console.log('🔧 First processed item:', {
          ...
          actualLabel: dataPoint.actualLabel,
          forecastLabel: dataPoint.forecastLabel,
  ...
  const fetchHistoricalData = useCallback(async () => {
    ...
    try {
      ...
      switch (selectedDataType) {
        case 'cot': endpoint = 'cot-history'; assetParam = `asset_code=${selectedAsset}`; break;
        case 'employment': endpoint = 'employment-history'; assetParam = `asset_code=${selectedAsset}`; break;
        case 'unemployment': endpoint = 'unemployment-history'; assetParam = `asset_code=${selectedAsset}`; break;
        case 'gdp': endpoint = 'gdp-history'; assetParam = `asset_code=${selectedAsset}`; break;
        case 'mpmi': endpoint = 'mpmi-history'; assetParam = `asset_code=${selectedAsset}`; break;
        case 'spmi': endpoint = 'spmi-history'; assetParam = `asset_code=${selectedAsset}`; break;
        case 'retail': endpoint = 'retail-sales-history'; assetParam = `asset_code=${selectedAsset}`; break;
        case 'inflation': endpoint = 'inflation-history'; assetParam = `asset_code=${selectedAsset}`; break;
        case 'interest': endpoint = 'interest-history'; assetParam = `asset_code=${selectedAsset}`; break;
        case 'nfp':
          if (selectedAsset !== 'USD') {
            setHistoricalData([]);
            setLastUpdated(new Date());
      ...
      const url = `http://${BASE_URL}:3000/api/economic-data/${endpoint}?${assetParam}&start_date=${formatDate(startDate)}&end_date=${formatDate(endDate)}&limit=500`;
      ...
      if (rawData.length === 0) {
        setError('No data returned from API for the selected criteria');
        setHistoricalData([]);
        setLastUpdated(new Date());
        return;
      }
      ...
      setHistoricalData(processedData);
      setLastUpdated(new Date());
      console.log('✅ Data processing complete:', processedData.length, 'points');
      
      if (processedData.length === 0) {
        setError('Data was returned but could not be processed - check field mappings');
      }
  ...
  }, [selectedAsset, selectedDataType, selectedTimeframe, BASE_URL]);
          ...
          <div style={styles.tooltipHeader}>
            ...
            {data.result && (
              <span style={{
                ...styles.resultBadge,
                backgroundColor: data.result === 'Beat' ? '#10b981' : 
                               data.result === 'Miss' ? '#ef4444' : '#3b82f6'
              }}>
                {data.result}
              </span>
            )}
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        <h1 style={styles.title}>Enhanced Trading Economics History</h1>
        <p style={styles.subtitle}>Comprehensive historical analysis with robust data extraction</p>
      </div>
      ...
      <div style={styles.statusBar}>
        <div style={styles.statusLeft}>
          <span style={styles.statusText}>
            📊 {config.label} for {selectedAsset} | {timeframes.find(t => t.value === selectedTimeframe)?.label}
          </span>
          {lastUpdated && (
            <span style={styles.statusTime}>
              Last updated: {lastUpdated.toLocaleString()}
        ...
        <div style={styles.statusRight}>
          ...
          {allFields.length > 0 && (
            <span style={styles.fieldCount}>
              {allFields.length} API fields detected
            </span>
          )}
        </div>
      </div>
          ...
          <div style={styles.chartWrapper}>
            <ResponsiveContainer width="100%" height={450}>
              ...
              >
                ...
                {/* Dark color for actual values */}
                <Bar 
                  dataKey="actual" 
                  fill={config.darkColor} 
                  name={historicalData[0]?.actualLabel || 'Actual'} 
                />
                
                {/* Light color for forecast values */}
                {historicalData[0]?.forecast !== undefined && historicalData[0]?.forecast !== 0 && (
                  <Bar 
                    dataKey="forecast" 
                    fill={config.lightColor} 
                    name={historicalData[0]?.forecastLabel || 'Forecast'} 
                  />
                )}
              </BarChart>
            </ResponsiveContainer>
      ...
      {/* Enhanced Debug Information with specific focus on problematic data types */}
      {(rawDataSample || allFields.length > 0) && (
        <div style={styles.debugInfo}>
          <h3>🔧 Enhanced Debug Information</h3>
          
          {/* Special debug section for inflation and interest data */}
          {(selectedDataType === 'inflation' || selectedDataType === 'interest') && rawDataSample && (
            <div style={styles.specialDebug}>
              <h4>🚨 Special Debug for {selectedDataType.toUpperCase()}</h4>
              <div style={styles.debugGrid}>
                <div>
                  <strong>Raw {selectedDataType} Data Fields:</strong>
                  <div style={styles.fieldGrid}>
                    {Object.keys(rawDataSample).map(field => (
                      <span 
                        key={field} 
                        style={{
                          ...styles.fieldBadge,
                          backgroundColor: field.toLowerCase().includes(selectedDataType === 'inflation' ? 'inflation' : 'interest') ||
                                          field.toLowerCase().includes(selectedDataType === 'inflation' ? 'cpi' : 'rate') ||
                                          field.toLowerCase().includes('forecast') ||
                                          field.toLowerCase().includes('actual') ||
                                          field.toLowerCase().includes('change') 
                                          ? '#dcfce7' : '#f1f5f9',
                          border: field.toLowerCase().includes(selectedDataType === 'inflation' ? 'inflation' : 'interest') ||
                                 field.toLowerCase().includes(selectedDataType === 'inflation' ? 'cpi' : 'rate') 
                                 ? '2px solid #10b981' : '1px solid #e2e8f0'
                        }}
                      >
                        {field}: {JSON.stringify(rawDataSample[field])}
                      </span>
                    ))}
                  </div>
                </div>
          ...
          <div style={styles.debugGrid}>
            {rawDataSample && (
              <div>
                <strong>Sample Raw API Response:</strong>
                <pre style={styles.debugCode}>
            ...
            {allFields.length > 0 && (
              <div>
                <strong>All Available Fields ({allFields.length}):</strong>
                <div style={styles.fieldGrid}>
                  {allFields.map(field => (
                    <span key={field} style={styles.fieldBadge}>
                      {field}
                    </span>
                  ))}
                </div>
            ...
            {historicalData.length > 0 && (
              <div>
                <strong>Processed Data Structure:</strong>
                <pre style={styles.debugCode}>
                  {JSON.stringify({
                    date: historicalData[0]?.date,
                    actual: historicalData[0]?.actual,
                    forecast: historicalData[0]?.forecast,
                    actualLabel: historicalData[0]?.actualLabel,
                    forecastLabel: historicalData[0]?.forecastLabel,
                    variance: historicalData[0]?.variance,
                    result: historicalData[0]?.result
                  }, null, 2)}
                </pre>
              </div>
            )}
      ...
      {!loading && historicalData.length > 0 && (
        <div style={styles.summaryContainer}>
          ...
          <div style={styles.summaryGrid}>
            ...
            <div style={styles.summaryCard}>
              <div style={styles.summaryLabel}>API Fields</div>
              <div style={styles.summaryValue}>{allFields.length}</div>
            </div>
            <div style={styles.summaryCard}>
              <div style={styles.summaryLabel}>Asset</div>
              <div style={styles.summaryValue}>{selectedAsset}</div>
            </div>
            <div style={styles.summaryCard}>
              <div style={styles.summaryLabel}>Data Type</div>
              <div style={styles.summaryValue}>{config.label}</div>
            </div>
            <div style={styles.summaryCard}>
              <div style={styles.summaryLabel}>Color Scheme</div>
              <div style={styles.summaryValue}>
                <span style={{color: config.darkColor}}>● Actual</span> | 
                <span style={{color: config.lightColor}}> ● Forecast</span>
              </div>
            </div>
          </div>
        </div>
