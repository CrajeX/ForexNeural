      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
        {!mobile && (
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                handleSettingChange("sidebarWidth", parseInt(e.target.value))
              }
              style={{ width: "100%" }}
            />
          </div>
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              handleSettingChange("cardBorderRadius", parseInt(e.target.value))
            }
            style={{ width: "100%" }}
          />
        </div>
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              handleSettingChange("cardPadding", parseInt(e.target.value))
            }
            style={{ width: "100%" }}
          />
        </div>
        ...
        >
          <span
            style={{ color: colors.text, fontSize: mobile ? "14px" : "16px" }}
          >
            Show Sidebar
          </span>
          <div
        ...
        >
          ...
          <div
            ...
            style={{
              width: "48px",
              height: "24px",
              backgroundColor: settings.compactMode
                ? colors.accent
                : colors.border,
              borderRadius: "12px",
              cursor: "pointer",
...
const TradingDashboard = () => {
  ...
  const getResponsiveFontSize = () => {
    ...
    return dashboardSettings.fontSize === "large"
      ? "18px"
  ...
  const dynamicStyles = {
    ...
    sidebar: {
      ...
      boxShadow:
        mobile && isMobileSidebarOpen ? "0 0 20px rgba(0,0,0,0.3)" : "none",
    },
    sidebarOverlay: {
      position: "fixed",
      top: 0,
    ...
    navButton: {
      ...
      padding: mobile
        ? "12px 16px"
        : `${Math.floor(responsivePadding / 2)}px ${responsivePadding}px`,
      borderRadius: `${dashboardSettings.cardBorderRadius}px`,
      textAlign: "left",
      border: "none",
      cursor: "pointer",
  ...
  const TopSetupsSection = () => {
      ...
      try {
        ...
        if (result.success && result.data) {
          ...
          const assetPairsWithMetrics = await Promise.all(
            result.data.slice(0, 10).map(async (pair) => {
              // Get more than 5 to sort and pick top 5
              try {
                ...
                return {
                  ...
                  change: `${metrics.totalScore >= 0 ? "+" : ""}${
                    metrics.totalScore
                  }`,
                  confidence: Math.min(
                    95,
                    Math.max(35, 50 + Math.abs(metrics.totalScore) * 3)
    ...
    const generateProperMetrics = (economicData) => {
      ...
      // COT Scoring
      const cotBaseScore =
        economicData.cotData.baseNetChangePercent > 0
          ? 1
          : economicData.cotData.baseNetChangePercent < 0
          ? -1
          : 0;
      const cotQuoteScore =
        economicData.cotData.quoteNetChangePercent > 0
          ? -1
          : economicData.cotData.quoteNetChangePercent < 0
          ? 1
          : 0;
