...
// ====================
// HISTORY DATA API ROUTES - Add these to your existing api-server.js
// ====================

// Save Retail Sentiment to History
app.post("/api/history/retail-sentiment", async (req, res) => {
  try {
    const { asset_pair_code, retailLong, retailShort } = req.body;

    console.log("📊 Saving Retail Sentiment to history for:", asset_pair_code);
    ...
    const [result] = await pool.execute(
      `INSERT INTO retail_sentiment_history 
       (asset_pair_code, retail_long, retail_short)
       VALUES (?, ?, ?)`,
      [asset_pair_code, parseFloat(retailLong), parseFloat(retailShort)]
    );

    console.log("✅ Retail Sentiment saved to history successfully");

    res.status(201).json({
      success: true,
      message: "Retail sentiment saved to history successfully",
      historyId: result.insertId,
    });
  } catch (error) {
    console.error("❌ Retail Sentiment history save error:", error);
    res.status(500).json({
      success: false,
      error: "Server error saving retail sentiment to history",
    });
  }
});

// Save COT Data to History
app.post("/api/history/cot", async (req, res) => {
  try {
    const { asset_code, cotLongContracts, cotShortContracts } = req.body;

    console.log("📊 Saving COT data to history for:", asset_code);
    ...
    const [result] = await pool.execute(
      ...
      [
        ...
        newLongContracts,
        newShortContracts,
        0, // change_in_long - can be calculated later
        0, // change_in_short - can be calculated later
        parseFloat(longPercent.toFixed(2)),
        parseFloat(shortPercent.toFixed(2)),
        netPosition,
        parseFloat(longPercent.toFixed(2)), // net_change_percent
      ]
    );

    console.log("✅ COT data saved to history successfully");

    res.status(201).json({
      success: true,
      message: "COT data saved to history successfully",
      historyId: result.insertId,
    });
  } catch (error) {
    console.error("❌ COT history save error:", error);
    res.status(500).json({
      success: false,
      error: "Server error saving COT data to history",
    });
  }
});

// Save Employment Change to History
app.post("/api/history/employment", async (req, res) => {
  try {
    const { asset_code, employeeChange, employeeChangeForecast } = req.body;

    console.log("📊 Saving Employment Change to history for:", asset_code);
    ...
    const [insertResult] = await pool.execute(
      `INSERT INTO employment_change_history 
       (asset_code, employment_change, forecast, net_change_percent, result)
       VALUES (?, ?, ?, ?, ?)`,
      [asset_code.toUpperCase(), newEmploymentChange, forecast, 0, result]
    );

    console.log("✅ Employment Change saved to history successfully");

    res.status(201).json({
      success: true,
      message: "Employment Change saved to history successfully",
      historyId: insertResult.insertId,
    });
  } catch (error) {
    console.error("❌ Employment Change history save error:", error);
    res.status(500).json({
      success: false,
      error: "Server error saving employment change to history",
    });
  }
});

// Save Unemployment Rate to History
app.post("/api/history/unemployment", async (req, res) => {
  try {
    const { asset_code, unemployment, unemploymentForecast } = req.body;

    console.log("📊 Saving Unemployment Rate to history for:", asset_code);
    ...
    const [insertResult] = await pool.execute(
      `INSERT INTO unemployment_rate_history 
       (asset_code, unemployment_rate, forecast, net_change_percent, result)
       VALUES (?, ?, ?, ?, ?)`,
      [asset_code.toUpperCase(), newUnemploymentRate, forecast, 0, result]
    );

    console.log("✅ Unemployment Rate saved to history successfully");

    res.status(201).json({
      success: true,
      message: "Unemployment Rate saved to history successfully",
      historyId: insertResult.insertId,
    });
  } catch (error) {
    console.error("❌ Unemployment Rate history save error:", error);
    res.status(500).json({
      success: false,
      error: "Server error saving unemployment rate to history",
    });
  }
});

// Save NFP Data to History
app.post("/api/history/nfp", async (req, res) => {
  try {
    const { asset_code, actualNfp, nfpForecast } = req.body;

    console.log("📊 Saving NFP data to history for:", asset_code);

    // Validation - Only allow USD
    if (asset_code.toUpperCase() !== "USD") {
      return res.status(400).json({
        success: false,
        error: "NFP data can only be saved for USD currency",
      });
    }
    ...
    const [insertResult] = await pool.execute(
      `INSERT INTO nfp_history 
       (asset_code, actual_nfp, forecast, net_change_percent)
       VALUES (?, ?, ?, ?)`,
      ["USD", parseFloat(actualNfp), parseFloat(nfpForecast), 0]
    );

    console.log("✅ NFP data saved to history successfully");

    res.status(201).json({
      success: true,
      message: "NFP data saved to history successfully",
      historyId: insertResult.insertId,
    });
  } catch (error) {
    console.error("❌ NFP history save error:", error);
    res.status(500).json({
      success: false,
      error: "Server error saving NFP data to history",
    });
  }
});

// Save GDP Growth to History
app.post("/api/history/gdp", async (req, res) => {
  try {
    const { asset_code, gdp, gdpForecast } = req.body;

    console.log("📊 Saving GDP Growth to history for:", asset_code);

    if (!asset_code || !gdp || !gdpForecast) {
      return res.status(400).json({
        success: false,
        error: "Asset code, GDP growth, and forecast are required",
      });
    }
    ...
    const [insertResult] = await pool.execute(
      `INSERT INTO gdp_growth_history 
       (asset_code, gdp_growth, forecast, change_in_gdp, result)
       VALUES (?, ?, ?, ?, ?)`,
      [asset_code.toUpperCase(), newGdpGrowth, forecast, 0, result]
    );

    console.log("✅ GDP Growth saved to history successfully");

    res.status(201).json({
      success: true,
      message: "GDP Growth saved to history successfully",
      historyId: insertResult.insertId,
    });
  } catch (error) {
    console.error("❌ GDP Growth history save error:", error);
    res.status(500).json({
      success: false,
      error: "Server error saving GDP growth to history",
    });
  }
});

// Save Manufacturing PMI to History
app.post("/api/history/mpmi", async (req, res) => {
  try {
    const { asset_code, mPMI, mPMIForecast } = req.body;

    console.log("📊 Saving Manufacturing PMI to history for:", asset_code);
    ...
    console.log("✅ Manufacturing PMI saved to history successfully");

    res.status(201).json({
      success: true,
      message: "Manufacturing PMI saved to history successfully",
      historyId: insertResult.insertId,
    });
  } catch (error) {
    console.error("❌ Manufacturing PMI history save error:", error);
    res.status(500).json({
      success: false,
      error: "Server error saving Manufacturing PMI to history",
    });
  }
});

// Save Services PMI to History
app.post("/api/history/spmi", async (req, res) => {
  try {
    const { asset_code, sPMI, sPMIForecast } = req.body;

    console.log("📊 Saving Services PMI to history for:", asset_code);
    ...
    console.log("✅ Services PMI saved to history successfully");

    res.status(201).json({
      success: true,
      message: "Services PMI saved to history successfully",
      historyId: insertResult.insertId,
    });
  } catch (error) {
    console.error("❌ Services PMI history save error:", error);
    res.status(500).json({
      success: false,
      error: "Server error saving Services PMI to history",
    });
  }
});

// Save Retail Sales to History
app.post("/api/history/retail", async (req, res) => {
  try {
    const { asset_code, retailSales, retailSalesForecast } = req.body;

    console.log("📊 Saving Retail Sales to history for:", asset_code);
    ...
    const [insertResult] = await pool.execute(
      `INSERT INTO retail_sales_history 
       (asset_code, retail_sales, forecast, net_change_percent, result)
       VALUES (?, ?, ?, ?, ?)`,
      [asset_code.toUpperCase(), newRetailSales, forecast, 0, result]
    );

    console.log("✅ Retail Sales saved to history successfully");

    res.status(201).json({
      success: true,
      message: "Retail Sales saved to history successfully",
      historyId: insertResult.insertId,
    });
  } catch (error) {
    console.error("❌ Retail Sales history save error:", error);
    res.status(500).json({
      success: false,
      error: "Server error saving Retail Sales to history",
    });
  }
});

app.post("/api/history/interest", async (req, res) => {
  try {
    ...
    if (recordId) {
      ...
      query = `SELECT asset_code, interest_rate, change_in_interest 
               FROM interest_rate 
               WHERE id = ?`;
      params = [recordId];
    } else {
      console.log("📋 Querying latest record (no ID provided)");
      query = `SELECT asset_code, interest_rate, change_in_interest 
               FROM interest_rate 
               ORDER BY id DESC 
               LIMIT 1`;
...
// Get COT Data History
app.get("/api/economic-data/cot-history", async (req, res) => {
  try {
    const { asset_code, start_date, end_date, limit = 100 } = req.query;

    console.log("📊 Fetching COT history for:", asset_code);
    ...
    let query = `
      SELECT * FROM cot_data_history 
      WHERE asset_code = ?
    `;
    let params = [asset_code.toUpperCase()];
    ...
    console.log(`✅ Retrieved ${data.length} COT history records`);
    ...
    console.error("❌ COT history fetch error:", error);
    res.status(500).json({
      success: false,
      error: "Server error fetching COT history",
    });
  }
});

// Get Employment Change History
app.get("/api/economic-data/employment-history", async (req, res) => {
  try {
    const { asset_code, start_date, end_date, limit = 100 } = req.query;

    console.log("📊 Fetching Employment Change history for:", asset_code);
    ...
    console.error("❌ Employment Change history fetch error:", error);
    res.status(500).json({
      success: false,
      error: "Server error fetching Employment Change history",
    });
  }
});

// Get Unemployment Rate History
app.get("/api/economic-data/unemployment-history", async (req, res) => {
  try {
    const { asset_code, start_date, end_date, limit = 100 } = req.query;

    console.log("📊 Fetching Unemployment Rate history for:", asset_code);
    ...
    let query = `
      SELECT * FROM unemployment_rate_history 
      WHERE asset_code = ?
    `;
    let params = [asset_code.toUpperCase()];
    ...
    console.error("❌ Unemployment Rate history fetch error:", error);
    res.status(500).json({
      success: false,
      error: "Server error fetching Unemployment Rate history",
    });
  }
});

// Get GDP Growth History
app.get("/api/economic-data/gdp-history", async (req, res) => {
  try {
    const { asset_code, start_date, end_date, limit = 100 } = req.query;

    console.log("📊 Fetching GDP Growth history for:", asset_code);
    ...
    let query = `
      SELECT * FROM gdp_growth_history 
      WHERE asset_code = ?
    `;
    let params = [asset_code.toUpperCase()];
    ...
    console.log(`✅ Retrieved ${data.length} GDP Growth history records`);
    ...
    console.error("❌ GDP Growth history fetch error:", error);
    res.status(500).json({
      success: false,
      error: "Server error fetching GDP Growth history",
    });
  }
});

// Get Manufacturing PMI History
app.get("/api/economic-data/mpmi-history", async (req, res) => {
  try {
    const { asset_code, start_date, end_date, limit = 100 } = req.query;

    console.log("📊 Fetching Manufacturing PMI history for:", asset_code);
    ...
    console.error("❌ Manufacturing PMI history fetch error:", error);
    res.status(500).json({
      success: false,
      error: "Server error fetching Manufacturing PMI history",
    });
  }
});

// Get Services PMI History
app.get("/api/economic-data/spmi-history", async (req, res) => {
  try {
    const { asset_code, start_date, end_date, limit = 100 } = req.query;

    console.log("📊 Fetching Services PMI history for:", asset_code);
    ...
    console.log(`✅ Retrieved ${data.length} Services PMI history records`);
    ...
    console.error("❌ Services PMI history fetch error:", error);
    res.status(500).json({
      success: false,
      error: "Server error fetching Services PMI history",
    });
  }
});

// Get Retail Sales History
app.get("/api/economic-data/retail-sales-history", async (req, res) => {
  try {
    const { asset_code, start_date, end_date, limit = 100 } = req.query;

    console.log("📊 Fetching Retail Sales history for:", asset_code);
    ...
    let query = `
      SELECT * FROM retail_sales_history 
      WHERE asset_code = ?
    `;
    let params = [asset_code.toUpperCase()];
    ...
    console.log(`✅ Retrieved ${data.length} Retail Sales history records`);
    ...
    console.error("❌ Retail Sales history fetch error:", error);
    res.status(500).json({
      success: false,
      error: "Server error fetching Retail Sales history",
    });
  }
});

// Get Core Inflation History
app.get("/api/economic-data/inflation-history", async (req, res) => {
  try {
    const { asset_code, start_date, end_date, limit = 100 } = req.query;

    console.log("📊 Fetching Core Inflation history for:", asset_code);
    ...
    let query = `
      SELECT * FROM core_inflation_history 
      WHERE asset_code = ?
    `;
    let params = [asset_code.toUpperCase()];
    ...
    console.log(`✅ Retrieved ${data.length} Core Inflation history records`);
    ...
    console.error("❌ Core Inflation history fetch error:", error);
    res.status(500).json({
      success: false,
      error: "Server error fetching Core Inflation history",
    });
  }
});

// Get Interest Rate History
app.get("/api/economic-data/interest-history", async (req, res) => {
  try {
    const { asset_code, start_date, end_date, limit = 100 } = req.query;

    console.log("📊 Fetching Interest Rate history for:", asset_code);
    ...
    console.log(`✅ Retrieved ${data.length} Interest Rate history records`);
    ...
    console.error("❌ Interest Rate history fetch error:", error);
    res.status(500).json({
      success: false,
      error: "Server error fetching Interest Rate history",
    });
  }
});

// Get NFP History (USD Only)
app.get("/api/economic-data/nfp-history", async (req, res) => {
  try {
    const { asset_code, start_date, end_date, limit = 100 } = req.query;

    console.log("📊 Fetching NFP history");

    // NFP is only for USD
    if (asset_code && asset_code.toUpperCase() !== "USD") {
      return res.status(400).json({
        success: false,
        error: "NFP data is only available for USD currency",
      });
    ...
    console.log(`✅ Retrieved ${data.length} NFP history records`);
    ...
    console.error("❌ NFP history fetch error:", error);
    res.status(500).json({
      success: false,
      error: "Server error fetching NFP history",
    });
  }
});

// Get Retail Sentiment History (for asset pairs)
app.get("/api/economic-data/retail-sentiment-history", async (req, res) => {
  try {
    const { asset_pair_code, start_date, end_date, limit = 100 } = req.query;

    console.log("📊 Fetching Retail Sentiment history for:", asset_pair_code);

    if (!asset_pair_code) {
      return res.status(400).json({
        success: false,
        error: "Asset pair code is required",
      });
    }

    let query = `
      SELECT * FROM retail_sentiment_history 
      WHERE asset_pair_code = ?
    `;
    let params = [asset_pair_code.toUpperCase()];
    ...
    console.log(`✅ Retrieved ${data.length} Retail Sentiment history records`);

    res.json({
      success: true,
      data: data,
      count: data.length,
      asset_pair_code: asset_pair_code.toUpperCase(),
    });
  } catch (error) {
    console.error("❌ Retail Sentiment history fetch error:", error);
    res.status(500).json({
      success: false,
      error: "Server error fetching Retail Sentiment history",
    });
  }
});
